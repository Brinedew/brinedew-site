...
const darkmodeInlinePath = new URL("../../components/scripts/darkmode.inline.ts", import.meta.url)
...
test("Iconoplasm first visit defaults to light theme instead of system dark", async () => {
  const head = await readFile(headPath, "utf8")
  const darkmodeInline = await readFile(darkmodeInlinePath, "utf8")
  ...
  assert.match(
    darkmodeInline,
    /isIconoplasmSurface\(\) \? "light" : userPref/,
    "the later dark-mode controller must not reapply OS dark mode on first Iconoplasm visit",
  )
  assert.match(
    darkmodeInline,
    /if \(isIconoplasmSurface\(\)\) return/,
    "Iconoplasm should ignore OS color-scheme changes until the user saves a site theme",
  )
  assert.match(
    darkmodeInline,
    /readThemeCookie\("brinedew_theme"\)\s*\|\|\s*localStorage\.getItem\("theme"\)/,
    "saved user theme choices still need to override the Iconoplasm first-visit default",
  )
})
...
test("gene route uses the shared detail cache instead of issuing raw duplicate fetches", async () => {
  ...
  assert.match(iconoplasmIndex, /data-icono-static-gene-shell="true"/)
  assert.match(iconoplasmIndex, /icono-gene-lead-card/)
  assert.match(iconoplasmIndex, /icono-card--variant-lab-label/)
  assert.match(iconoplasmIndex, /iconoplasm-static-gene-shell:start/)
  assert.match(iconoplasmIndex, /iconoplasm-static-gene-shell:end/)
...
test("gene request summary and image edit providers are single-flight route dependencies", async () => {
  ...
  assert.match(app, /singleFlightQuery\("image-edit-providers"/)
  assert.match(app, /imageEditProvidersCache = payload \|\| \{\}/)
  assert.match(
    app,
  ...
  assert.match(requestPanelBlock, /fetchImageEditProviders\(\)/)
  assert.doesNotMatch(
    requestPanelBlock,
