...
test("admin finalization enqueue and kick both send only the canonical Queue drain message", async () => {
  ...
  const enqueueResponse = await handleIconoplasmRequestInsideTheOnlyAllowedInternalStatefulWorkerDoNotDuplicate(
  ...
  const kickResponse = await handleIconoplasmRequestInsideTheOnlyAllowedInternalStatefulWorkerDoNotDuplicate(
...
test("Iconoplasm sync finalization keeps one true Queue path and no direct processor", () => {
  ...
  assert.match(workerSource, /durable D1 ledger rows are advanced by Cloudflare Queue messages of kind/)
