...
- An assignment owns a bounded state machine: `pending_acceptance` -> `active` <->
  `suspended` -> `ended`. `ended` is terminal. A new tenure receives a new ID.
...
| Pending invitation is declined or cancelled         | Assignment ends without a leave policy and both uniqueness slots reopen           |
| Second open assignment for a gene or account        | Refused even when either assignment is only awaiting acceptance                   |
| Offer targets a gene without a verified seed        | Refused; migration never invents source prose                                     |
