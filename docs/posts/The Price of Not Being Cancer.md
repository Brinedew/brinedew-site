...
There's another clever trick: starting each generation from a single cell. The fertilized egg is a "clonal reset" that purges almost all the somatic variation that built up in the parent organisms. Any mutation present in that single starting cell faces brutal early developmental selection - if it's harmful enough, the embryo simply doesn't develop.
