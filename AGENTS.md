...
The **only canonical blot publication path** verifies immutable `blots/v1/...` bytes in `workers/lib/iconoplasm-card-publication.js`, then advances the published card head. The existing Worker resolves the stable first-party `/blot/{symbol}.webp` route from that card. Do not recreate the old mutable Bunny `/blot/{symbol}.webp` writer or its admin backfill: its cached placeholder blocked a real candidate while the first-party route already served the correct image (2026-09-24).
