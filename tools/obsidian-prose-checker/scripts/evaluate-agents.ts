...
const report: AgentEvaluation[] = evaluatedAgents.map((agent) => {
  ...
  const averageFalsePositiveRate = average(
    negativeRuns.map((run) => run.metrics.falsePositiveRate),
  );
...
const outputRoot = resolve(process.cwd(), "..", "..", "artifacts", "obsidian-prose-checker", "evaluations");
await mkdir(outputRoot, { recursive: true });
const outputPath = resolve(outputRoot, `evaluation-${new Date().toISOString().replace(/[:.]/g, "-")}.json`);
await writeFile(
  outputPath,
  `${JSON.stringify(
    {
