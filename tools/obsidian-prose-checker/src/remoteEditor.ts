  ...
  update(previous, transaction) {
    ...
    for (const effect of transaction.effects) {
      if (effect.is(setAllFindingsEffect)) {
        ...
        findings = [...findings.filter((finding) => finding.id !== effect.value.id), effect.value]
      }
    }
...
function renderFindingCard(
  ...
  if (finding.replacement !== null) {
    ...
    replacementText.textContent =
      finding.replacement === "" ? "Delete this passage" : finding.replacement
...
export class RemoteEditorBridge {
  ...
  replaceAgentFindings(filePath: string, agentId: string, findings: ResolvedFinding[]): void {
    for (const view of this.views) {
