  ...
  return new Promise((resolve, reject) => {
    ...
    request.on("error", (error: Error) => {
      ...
      if (error instanceof OpenCodeError) {
        ...
        finishReject(
          new OpenCodeError(error.message || "OpenCode network request failed.", "network", null, null, true),
        );
      }
    });
    request.on("close", () => {
...
function parseFindingsJson(text: string): AgentFinding[] {
  ...
  if (!Array.isArray(findings) || !findings.every(isAgentFinding)) {
    throw new OpenCodeError("Agent response does not match the findings schema.", "invalid-agent-schema");
  }
  return findings;
}
...
export class OpenCodeClient {
  ...
  async probeModel(signal: AbortSignal, force = false): Promise<ModelCatalogInfo> {
    ...
    try {
      ...
      const record = records?.find(
        (item) => (item.id ?? item.model ?? item.name ?? "").toLowerCase() === this.model.toLowerCase(),
      );
      this.catalogCache = {
  ...
  async runAgent(
    ...
    try {
      ...
      if (!(error instanceof OpenCodeError) || !["malformed-agent-json", "invalid-agent-schema"].includes(error.code)) {
        throw error;
      }
