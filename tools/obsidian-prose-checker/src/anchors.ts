...
function intersectsDisallowedProtectedRange(
  ...
  return ranges.some(
    (range) => from < range.to && to > range.from && range.kinds.some((kind) => !allowed.has(kind)),
  )
}
...
export function resolveAnchor(text: string, finding: AgentFinding): AnchorResolution | null {
  ...
  if (
    ...
    if (from === undefined || !hasDisambiguatingContext || !contextMatches(text, from, finding))
      return null
    return { from, to: from + finding.exactText.length }
  }
  return null
...
function findingId(finding: AgentFinding, from: number, to: number): string {
  return createHash("sha256")
    .update(
      [finding.agentId, String(from), String(to), finding.exactText, finding.explanation].join(
        "\u0000",
      ),
    )
    .digest("hex")
