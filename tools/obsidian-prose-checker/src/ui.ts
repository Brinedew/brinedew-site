...
export class ProseProgressView extends ItemView {
  ...
  private render(): void {
    ...
    if (!snapshot) {
      container.createDiv({
        cls: "bpc-progress-empty",
        text: "No prose check has run in this session.",
      })
      return
    }
    ...
    for (const state of snapshot.agents) {
      ...
      if (state.status === "queued" || state.status === "running") {
        ...
        cancel.addEventListener("click", () =>
          this.coordinator.cancelAgent(snapshot.runId, state.agentId),
        )
        ...
        retry.addEventListener("click", () =>
          this.coordinator.retryAgent(snapshot.runId, state.agentId),
        )
      } else {
        const rerun = actions.createEl("button", {
          attr: { "aria-label": `Run ${state.label} again` },
        })
        setIcon(rerun, "play")
