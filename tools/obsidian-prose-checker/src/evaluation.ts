...
export function buildEvaluationFixtures(agent: AgentDefinition): EvaluationFixture[] {
  ...
  const neighborNegatives = neighborExamples(agent)
    .slice(0, 8)
    .map((example, index) => ({
  ...
  const ownNegatives = Array.from({ length: 8 }, (_, index) => ({
    ...
    kind: "negative" as const,
    text:
      index === 0 ? seedNegative.text : `${NEUTRAL_PASSAGES[index - 1] ?? ""} ${seedNegative.text}`,
...
export function assertEvaluationCorpus(): void {
  ...
  for (const agent of AGENTS) {
    ...
    if (positives.length < 12 || negatives.length < 24) {
      throw new Error(
        `${agent.id} has ${positives.length} positives and ${negatives.length} negatives.`,
      )
    }
  }
}
