...
**CRITICAL: Always use CSS variables for colors** - never hardcode hex values. The site defines color variables that automatically switch between light and dark modes.

**Brand colors (from BrinedewStyle/Visual/BrinedewVisualStyleGuide.md):**

- **Accent (teal)**: `oklch(50% 0.08 185)` light, `oklch(68% 0.08 185)` dark
- **Background**: Warm cream in light mode, deep brown-black in dark
- **Text**: Rich brown in light mode, warm white in dark
- **Secondary text**: Muted brown/warm gray

**Defined CSS variables (from quartz/util/theme.ts):**

```css
--light       /* Page background */
--lightgray   /* Secondary backgrounds, input fields */
--gray        /* Borders, disabled elements */
--darkgray    /* Body text, default button/icon color */
--dark        /* Headings, emphasized text */

--highlight   /* Subtle backgrounds */
--textHighlight /* Text selection */
```
...
4. Confirm teal accent color appears, not blue or black
...
- The Iconoplasm app-owned budget path (the operation-cost ledger in `workers/iconoplasm/operation-cost-*.js` and the card-publication coordinator's write reservations) is a mutation and publication barrier, not a hot-read dependency. Hot public and first-party reads must stay off that request path entirely; dangerous admin mutation or publication families should fail closed through it.
- There is no separate card-catalog budget preflight. `assertIconoplasmCardCatalogBudgetPreflight` was defined but never called anywhere in this repository's history, and was deleted on 2026-09-26 with its test and config flag (B-869).
