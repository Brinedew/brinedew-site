...
CREATE TRIGGER icono_event_archive_no_update
BEFORE UPDATE ON icono_manifestation_events
BEGIN
  SELECT RAISE(ABORT, 'event_archive_is_immutable');
END;

CREATE TRIGGER icono_event_archive_no_delete
BEFORE DELETE ON icono_manifestation_events
BEGIN
  SELECT RAISE(ABORT, 'event_archive_is_immutable');
END;

CREATE TRIGGER icono_event_archive_no_insert_after_seal
...
CREATE TRIGGER icono_event_archive_manifest_immutable_delete
