...
function summarize(rows) {
  const playwrightRows = rows.filter((row) => /@playwright[\\/]+mcp|playwright-mcp/i.test(row.CommandLine || ""))
  const nodeReplRows = rows.filter((row) => row.Name === "node_repl.exe")
  const launchForms = new Set(
    playwrightRows.map((row) =>
      /playwright-mcp/i.test(row.CommandLine || "") && !/@playwright[\\/]+mcp/i.test(row.CommandLine || "")
        ? "playwright-mcp"
        : "@playwright/mcp",
    ),
  )
