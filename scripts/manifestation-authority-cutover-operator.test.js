...
test("sharded operator accepts omitted and singleton shard selections without scalar unwrapping", async () => {
  ...
  assert.match(text, /\[ValidateSet\(2, 4, 8, 16\)\]/)
  assert.match(text, /\[ValidateRange\(1, 25\)\]/)
  assert.match(text, /\[int\] \$PageLimit = 5/)
  assert.match(text, /\[ValidateRange\(30, 180\)\]/)
  assert.match(text, /\[int\] \$RequestTimeoutSeconds = 120/)
