...
function source(t, onRead = async (rows) => rows) {
  ...
  t.after(() => db.close())
  db.exec(
    "CREATE TABLE icono_image_votes(gene_symbol TEXT,user_id TEXT,asset_sha256 TEXT,vote_value INTEGER,PRIMARY KEY(gene_symbol,user_id,asset_sha256))",
  )
  let reads = 0
  const binding = {
    prepare(query) {
        ...
        bind(...args) {
          return {
            async all() {
              ...
              return { results: await onRead(rows, ++reads, db) }
            },
            async first() {
...
function seedVotes(f, s, count) {
  ...
  f.sql.db
    .prepare("UPDATE asset_summary SET upvotes=?,vote_count=?,score=? WHERE asset_sha256=?")
    .run(count, count, count, sha("a"))
  f.sql.db
    .prepare("UPDATE vision_summary SET upvotes=?,vote_count=?,score=? WHERE vision_id=?")
    .run(count, count, count, "anima-v1-9")
}
...
test("CONTROL: matching stable source can activate with coherent selection", async (t) => {
  ...
  const response = await post(f.coordinator, "/authority/activate", activation()),
    body = await response.json()
  const state = f.coordinator.publication.read()
  t.diagnostic(
    JSON.stringify({
      case: "stable_source_control",
      status: response.status,
      pages: s.reads(),
      body,
      selection: state?.selectionRef,
    }),
  )
...
test("TRANSFER: a changed already-read source row cannot disappear across OFFSET pages", async (t) => {
  ...
  const s = source(t, async (rows, page, db) => {
    if (page === 1)
      db.prepare("UPDATE icono_image_votes SET vote_value=-1 WHERE user_id=?").run("reader-00000")
    return rows
  })
  seedVotes(f, s, 501)
  f.coordinator.env = { ICONOPLASM_DB: s.binding }
  const response = await post(f.coordinator, "/authority/activate", activation()),
    body = await response.json()
  const accepted = s.db
    .prepare("SELECT vote_value FROM icono_image_votes WHERE user_id=?")
    .get("reader-00000").vote_value
  const imported = f.sql.db
    .prepare("SELECT vote_value FROM vote_by_user_asset WHERE user_id=?")
    .get("reader-00000").vote_value
  t.diagnostic(
    JSON.stringify({
      case: "source_change_between_pages",
      status: response.status,
      pages: s.reads(),
      accepted,
      imported,
      epoch: f.coordinator.getMeta("authority_epoch"),
      body,
    }),
  )
  assert.ok(
    response.status >= 400 || accepted === imported,
    "Live OFFSET reads certified a changed source as a complete snapshot",
  )
})
...
test("TRANSFER: vote accepted during source verification cannot leave the old selection clean at activation", async (t) => {
  ...
  const s = source(t, async (_rows, page, db) => {
    ...
    return db
      .prepare(
        "SELECT user_id,asset_sha256,vote_value FROM icono_image_votes ORDER BY user_id,asset_sha256",
      )
      .all()
  })
  f.coordinator.env = { ICONOPLASM_DB: s.binding }
  ...
  let voteResponse
  try {
    voteResponse = await post(f.coordinator, "/vote/set", {
      asset_sha256: sha("b"),
      user_id: "new-reader",
      vote_value: 1,
      vision_id: "anima-v1-8",
    })
    s.db
      .prepare("INSERT INTO icono_image_votes VALUES (?,?,?,?)")
      .run("TP53", "new-reader", sha("b"), 1)
  } finally {
    release.resolve()
  }
  const response = await activating,
    body = await response.json()
  assert.equal(voteResponse.status, 200, "Setup must accept the concurrent vote")
  const state = f.coordinator.publication.read(),
    winner = f.coordinator.geneAuthorityWinner().winner?.asset_sha256
  const unsettled = f.coordinator.pendingOutboxRows(10).length
  t.diagnostic(
    JSON.stringify({
      case: "vote_during_activation",
      status: response.status,
      body,
      winner,
  ...
  assert.ok(
    response.status >= 400 ||
      (state?.pending === true && unsettled === 0) ||
...
test("SUPERVOTE: clearing an accepted supervote must not commit ahead of a failed v2 publication intent", async (t) => {
  ...
  c.caretakerSupervotes.projectAssetEligibility({
    event_id: "eligibility-setup",
    source_event_sequence: 101,
    gene_symbol: "TP53",
    asset_sha256: sha("b"),
  ...
  await c.caretakerSupervotes.setSelection({
    accountId: "reader-1",
    assetSha256: sha("b"),
    direction: 1,
    commandId: "setup-supervote",
    requestSha256: sha("1"),
  ...
  const before = c.caretakerSupervotes.readHead(),
    outboxBefore = c.caretakerSupervotes.pendingOutboxRows(100).length
  c.publication.commitSelection = async () => {
    throw Error("Audit: publication intent storage unavailable")
  }
  await assert.rejects(
    post(c, "/caretaker-supervote/set", {
      account_id: "reader-1",
      asset_sha256: null,
      command_id: "clear-supervote",
      request_sha256: sha("2"),
      expected_assignment_version: 1,
      expected_supervote_version: before.supervote_version,
    }),
    /publication intent storage unavailable/,
  )
  const after = c.caretakerSupervotes.readHead(),
    state = c.publication.read()
  t.diagnostic(
    JSON.stringify({
      case: "supervote_intent_gap",
      before,
      after,
      pending: state.pending,
      selectionRef: state.selectionRef,
      newLegacyCaretakerRows: c.caretakerSupervotes.pendingOutboxRows(100).length - outboxBefore,
    }),
  )
  assert.deepEqual(
    after,
    before,
    "The assignment path is atomic, but the supervote path still commits first",
  )
})
