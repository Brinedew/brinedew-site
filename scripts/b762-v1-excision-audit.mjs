...
const call = (coordinator, path, body) =>
  coordinator.fetch(
...
async function dirty(t) {
  ...
  assert.equal(
    f.coordinator.publication.read().pending,
    true,
    "Setup must create pending v2 publication",
  )
  return f
}
...
test("EXCISION: an activated v2 vote must not append the old D1/Queue outbox", async (t) => {
  ...
  t.diagnostic(
    JSON.stringify({
      case: "v2_legacy_outbox",
      authority: outcome.authority,
      voteChanged: outcome.vote.changed,
      publicationChanged: outcome.publication.changed,
      legacyRows,
    }),
  )
...
test("LIVENESS: actual alarm must process publication-only pending work", async (t) => {
  ...
  let adapterCalls = 0
  coordinator.genePublicationAdapter = () => async (ticket) => {
    adapterCalls++
    return uploadedFor(ticket)
  }
  const result = await coordinator.alarm()
  t.diagnostic(
    JSON.stringify({
      case: "publication_only_alarm",
      result,
      adapterCalls,
      stillPending: coordinator.publication.read().pending,
    }),
  )
...
test("MIGRATION: oversized candidate batches must reject before silently dropping candidates", async (t) => {
  ...
  const items = Array.from({ length: 65 }, (_, i) =>
    candidate((i + 1).toString(16).padStart(64, "0")),
  )
  ...
  t.diagnostic(
    JSON.stringify({
      case: "65_candidate_import",
      status: response.status,
      reported: body.candidate_count,
      stored,
    }),
  )
...
test("MIGRATION: missing items must not delete the complete candidate authority", async (t) => {
  ...
  t.diagnostic(
    JSON.stringify({ case: "missing_candidate_payload", status: response.status, remaining }),
  )
...
test("CANON: withdrawing the current winner must atomically mark replacement pending", async (t) => {
  const { coordinator } = await seeded(t)
  const response = await call(coordinator, "/authority/candidates", {
    items: [candidate(sha("a"), { status: "rejected", autopick_eligible: 0 }), candidate(sha("b"))],
  })
  const body = await response.json()
  ...
  t.diagnostic(
    JSON.stringify({
      case: "withdrawal",
      status: response.status,
      winner: body.winner_asset_sha256,
...
test("MIGRATION: activation must bind the seeded pointer to its administrator override", async (t) => {
  ...
  coordinator.importGeneCandidateAuthority([
    candidate(sha("a"), { created_at: "2026-01-02" }),
    candidate(sha("b"), { created_at: "2026-01-01" }),
  ])
  const response = await call(coordinator, "/authority/activate", {
    ...
    published: {
      ...
      object_key: "published-cards/v2/immutable/cards/seed.json",
    },
  })
  const body = await response.json()
  const state = coordinator.publication.read()
  t.diagnostic(
    JSON.stringify({
      case: "activation_override",
      status: response.status,
      reported: body.winner_asset_sha256,
      actual: coordinator.geneAuthorityWinner().winner?.asset_sha256,
      selectionRef: state?.selectionRef,
    }),
  )
  assert.equal(response.status, 200)
  assert.ok(
    state.selectionRef.includes(`winner=${sha("b")}`),
    "Seeded clean pointer names the pre-override election",
  )
...
test("EXCISION: an empty eligible set must not silently downgrade an activated v2 gene", async (t) => {
  ...
  t.diagnostic(
    JSON.stringify({
      case: "v2_authority_downgrade",
      authority: outcome.authority,
      epoch: coordinator.getMeta("authority_epoch"),
    }),
  )
...
test("COST: duplicate v2 votes must not export every historical asset summary", async (t) => {
  ...
  coordinator.exportAssetSummaries = () => {
    exports++
    return original()
  }
  const outcome = await coordinator.applyAuthoritativeVoteMutation(
  ...
  t.diagnostic(
    JSON.stringify({
      case: "duplicate_vote_history_exports",
      voteChanged: outcome.vote.changed,
      exports,
    }),
  )
...
test(
  "COST RECEIPTS: measure actual legacy and v2 routes in native workerd",
  { timeout: 120000 },
  async (t) => {
  ...
  `,
    ...
    assert.ok(
      large.duplicate.cost.rows_read <= small.duplicate.cost.rows_read + 200,
      "Activated v2 reintroduces history-dependent reads even for duplicate votes: " +
