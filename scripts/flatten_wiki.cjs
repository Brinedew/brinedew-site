...
const BACKUP_PATTERNS = [/\.md\.backup$/i, /\.backup$/i, /~$/, /\.bak$/i, /\.orig$/i]
...
;(async function main() {
  ...
  for (const rel of all) {
    ...
    if (JSON.stringify(beforeAliases) !== JSON.stringify(newAliases)) {
      report.aliasChanges.push({
        file: rel,
        added: newAliases.filter((a) => !beforeAliases.includes(a)),
      })
    }
    ...
    const finalStr = matter.stringify(
      body,
      { ...fm.data, tags: afterTags, aliases: newAliases },
      { lineWidth: 10000 },
    )
    ...
    if (occ) {
      if (occ.hash === thisHash) {
        ...
        report.conflicts.push({
          ...
          a: occ.srcRel,
          b: rel,
          parked: [fromRoot(alt1), fromRoot(alt2)],
        })
        if (!DRY_RUN) {
