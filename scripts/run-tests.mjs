...
function trackedFiles() {
  try {
    ...
    return walk(process.cwd()).map((file) => path.relative(process.cwd(), file).replaceAll(path.sep, "/"))
  }
}
...
const testFiles = explicitTestArgs.length > 0 ? explicitTestArgs : trackedFiles().filter(isSourceTest)
