...
async function writeSnapshotFiles(rootDir, snapshot) {
  ...
  const jsonTargetPath = path.join(generatedDir, "iconoplasm-observability-snapshot.json")
  const fileContents = [
  ...
  await writeFile(jsonTargetPath, `${JSON.stringify(snapshot)}\n`, "utf8")
  console.log(`Wrote ${targetPath}`)
  console.log(`Wrote ${jsonTargetPath}`)
}
