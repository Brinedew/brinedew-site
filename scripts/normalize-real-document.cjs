...
class LineageNormalizer {
  constructor() {
    ...
    this.MARKER_RX =
      /<span(?=[^>]*\bdata-lineage-uid=["']?([0-9a-fA-F-]{8,})["']?)(?=[^>]*\bdata-lineage-section=["']?([0-9]+(?:\.[0-9]+)*)["']?)[^>]*>\s*<\/span>/gi
    this.SECTION_ONLY_RX =
      /<span(?![^>]*\bdata-lineage-uid=)[^>]*\bdata-lineage-section=["']?([0-9]+(?:\.[0-9]+)*)["']?[^>]*>\s*<\/span>/gi
  }
  ...
  emitSection(section) {
    ...
    return (
      `<span data-lineage-uid="${section.uid}" data-lineage-section="${section.path}"></span>\n` +
      canon.trimEnd() +
      "\n\n"
    )
  }
...
// Analyze structure
const scaffoldMatches = normalized.match(
  /<!-- lineage:scaffold start -->([\s\S]*?)<!-- lineage:scaffold end -->/,
)
const contentMatches = normalized.match(
  /<!-- lineage:content start -->([\s\S]*?)<!-- lineage:content end -->/,
)

if (scaffoldMatches && contentMatches) {
  const scaffoldSections = (scaffoldMatches[1].match(/<span[^>]*data-lineage-section/g) || [])
    .length
