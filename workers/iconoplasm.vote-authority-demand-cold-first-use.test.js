...
test("cold ordinary first mutation reconstructs retained candidate authority without test-side seeding", async (t) => {
  ...
  const coordinator = new IconoplasmVoteCoordinator(state, { ICONOPLASM_DB: retainedLegacyD1(symbol) })
  await state.ready
  ...
  assert.equal(status.candidate_count, 1, "cold handover did not reconstruct retained candidate authority")
  assert.equal(
