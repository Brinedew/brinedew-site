...
test("publication alias manifest is deterministic and small enough to inline", async () => {
  ...
  assert.equal(first.alias_count, 29)
  assert.ok(
...
test("server-side gene views merge aliases without mutating the immutable base artifact", () => {
  const baseGene = Object.freeze({ s: "RELA", n: "RELA proto-oncogene", a: ["NFKB3"] })
  const merged = mergeIconoplasmPublicationAliasesIntoGene(
    baseGene,
    "RELA",
    ICONOPLASM_PUBLICATION_ALIASES,
  )
