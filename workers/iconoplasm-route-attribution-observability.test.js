...
async function withCapturedRouteLogs(fn) {
  ...
  console.log = (...args) => {
    ...
    if (typeof first === "string") {
      try {
        ...
        if (parsed?.service === "iconoplasm" && parsed?.route) logs.push(parsed)
      } catch {}
    }
  }
  try {
...
test("successful sync-style admin mutations emit one Workers Observability route log", async () => {
  ...
  assert.equal(logs[0]?.route, "admin_catalog_upsert")
  assert.equal(logs[0]?.status, 200)
  assert.equal(logs[0]?.rows_written_request, 2)
...
test("fail-closed limiter rejections still emit one Workers Observability route log", async () => {
  ...
  assert.equal(logs.length, 1)
  assert.equal(logs[0]?.route, "admin_catalog")
  assert.equal(logs[0]?.outcome_class, "limited")
  assert.equal(logs[0]?.limiter_stage, "preflight")
  assert.equal(logs[0]?.status, 503)
  assert.equal(logs[0]?.target_cap_reached, true)
})

test("public handled routes emit request-count route logs without touching D1 limiter", async () => {
  const { result: response, logs } = await withCapturedRouteLogs(() =>
    handleIconoplasmRequestInsideTheOnlyAllowedInternalStatefulWorkerDoNotDuplicate(
      new Request(
        "https://the-only-allowed-internal-stateful-worker-do-not-duplicate/api/public/v1/genes/INS",
        {
          method: "GET",
          headers: {
  ...
  assert.equal(logs.length, 1)
  assert.equal(logs[0]?.route, "public_gene_detail")
  assert.equal(logs[0]?.budget_class, "public_read")
  assert.equal(logs[0]?.source_class, "first_party_site")
  assert.equal(logs[0]?.outcome_class, "client_error")
  assert.equal(logs[0]?.status, 403)
  assert.equal(logs[0]?.request_count, 1)
})
