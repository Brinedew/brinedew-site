...
test("iconoplasm legacy admin path redirects to the apex-hosted ops page instead of going dead", async () => {
  ...
  const response = await worker.fetch(
    new Request("https://iconoplasm.brinedew.bio/admin/iconoplasm", {
      method: "GET",
      redirect: "manual",
    }),
    {},
...
test("admin canon audit derives portrait URLs from asset sha even when legacy key copies are blank", async () => {
  ...
  assert.equal(
    payload?.rows?.[0]?.current?.medium_url,
    `https://iconoplasm.brinedew.bio/portraits/v1/${"a".repeat(2)}/${"a".repeat(64)}/medium.webp`,
  )
  assert.equal(
    payload?.rows?.[0]?.leader?.medium_url,
    `https://iconoplasm.brinedew.bio/portraits/v1/${"b".repeat(2)}/${"b".repeat(64)}/medium.webp`,
  )
})
...
test("admin assets list derives portrait URLs from asset sha instead of copied key columns", async () => {
  ...
  assert.equal(response.status, 200)
  assert.equal(
    payload?.assets?.[0]?.medium_url,
    `https://iconoplasm.brinedew.bio/portraits/v1/${"c".repeat(2)}/${"c".repeat(64)}/medium.webp`,
  )
  assert.equal(
    payload?.assets?.[0]?.thumb_url,
    `https://iconoplasm.brinedew.bio/portraits/v1/${"c".repeat(2)}/${"c".repeat(64)}/thumb.webp`,
  )
})
...
test("admin asset storage audit consumes queued work and refreshes the persisted truth summary", async () => {
  ...
  class FakeStatement {
    ...
    async all() {
      if (
        this.sql.includes("FROM icono_storage_audit_queue q") &&
        this.sql.includes("q.audit_state = 'unknown'")
      ) {
        return {
    ...
    async run() {
      if (
        this.sql.includes("INSERT OR IGNORE INTO icono_storage_audit_queue") &&
        this.sql.includes("last_audited_at")
      ) {
        return { success: true }
      }
      if (
        this.sql.includes("UPDATE icono_storage_audit_queue") &&
        this.sql.includes("last_audited_at = COALESCE")
      ) {
        const payload = JSON.parse(String(this.args[0] || "[]"))
        for (const item of Array.isArray(payload) ? payload : []) {
          const row = this.db.queueRows.find(
            (candidate) =>
              candidate.gene_symbol === item.gene_symbol &&
              candidate.asset_sha256 === item.asset_sha256,
          )
          if (!row) {
      ...
      if (
        this.sql.includes("INSERT INTO icono_storage_audit_queue") &&
        this.sql.includes("last_audited_at")
      ) {
        const payload = JSON.parse(String(this.args[0] || "[]"))
        for (const item of Array.isArray(payload) ? payload : []) {
