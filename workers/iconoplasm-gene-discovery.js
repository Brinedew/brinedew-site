...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{background:var(--paper);color:var(--ink);font-family:IconoMono,ui-monospace,monospace;line-height:1.5}body{margin:0}
    ...
    .range-summary{display:flex;flex-wrap:wrap;gap:8px 24px;align-items:baseline;padding:24px 0;border-bottom:1px solid var(--line);color:var(--muted)}.range-summary strong{color:var(--ink)}.gene-list{display:grid;grid-template-columns:1fr;margin:0;padding:18px 0 0;list-style:none}.gene-list li{border-bottom:1px solid color-mix(in srgb,var(--line) 65%,transparent)}.gene-link{display:grid;grid-template-columns:minmax(9ch,.45fr) minmax(0,1fr) minmax(0,auto);gap:16px;align-items:center;min-height:52px;padding:9px 4px;text-decoration:none}.gene-symbol{font-weight:700;color:var(--accent)}.gene-name{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:.78rem;color:var(--muted)}.gene-caretaker{display:inline-flex;align-items:center;justify-self:end;gap:7px;min-width:0;font-size:.76rem;color:var(--ink)}.gene-caretaker-avatar{display:block;width:28px;height:28px;flex:0 0 auto;border-radius:50%;object-fit:cover}.gene-caretaker-name{max-width:13ch;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .archive-foot{padding:28px 0;color:var(--muted);font-size:.76rem}.archive-foot code{color:var(--ink)}
    @media(max-width:720px){.archive-shell{width:min(100% - 22px,1120px);padding-top:10px}.letter-section{grid-template-columns:44px 1fr;gap:10px}.letter-section h2{font-size:2.2rem}.range-list,.gene-list{grid-template-columns:1fr}.archive-head h1{font-size:clamp(2.7rem,17vw,5.2rem)}.gene-link{grid-template-columns:minmax(7ch,.45fr) minmax(0,1fr) minmax(0,auto);gap:10px}.gene-caretaker-name{max-width:9ch}}
...
export function renderIconoplasmGeneRangeHtml(snapshot, range, projection, caretakers = {}) {
  ...
  const links = projectedGenes
    .map(({ gene }) => {
      ...
      const caretaker = caretakers?.[gene.symbol]
      const caretakerHtml =
        caretaker?.username && caretaker?.avatar_url
          ? `<span class="gene-caretaker" aria-label="Caretaker ${escapeHtml(caretaker.username)}"><img class="gene-caretaker-avatar" src="${escapeHtml(caretaker.avatar_url)}" alt="" loading="lazy" decoding="async"><span class="gene-caretaker-name">${escapeHtml(caretaker.username)}</span></span>`
          : '<span class="gene-caretaker"></span>'
      return `<li class="gene-entry"><a class="gene-link" href="${path}"><span class="gene-symbol">${escapeHtml(gene.symbol)}</span><span class="gene-name">${escapeHtml(gene.fullName)}</span>${caretakerHtml}</a></li>`
    })
    .join("")
