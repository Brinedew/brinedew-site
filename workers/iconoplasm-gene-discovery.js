...
export function renderIconoplasmGeneRangeHtml(snapshot, range, projection, caretakers = {}) {
  ...
  const links = projectedGenes
    .map(({ gene }) => {
      ...
      return `<li class="gene-entry"><a class="gene-link" href="${path}"><span class="gene-identity"><span class="gene-symbol">${escapeHtml(gene.symbol)}</span><span class="gene-name">${escapeHtml(gene.fullName)}</span></span>${caretakerHtml}</a></li>`
    })
    .join("")
