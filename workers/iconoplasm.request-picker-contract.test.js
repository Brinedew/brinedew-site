...
test("Iconoplasm request picker uses an explicit combobox/listbox contract", () => {
  ...
  assert.match(app, /String\(config\.placeholder \|\| "pick an emulsion"\)/, "request picker should keep the short placeholder copy")
  assert.doesNotMatch(app, /Search emulsion code or vision ID\. Leave blank for random\./, "request picker should not ship the old verbose placeholder copy")
  ...
  assert.match(app, /function compareRequestOptionStrength\(left, right\)/, "request picker should preserve vote-strength ordering after filtering")
  assert.match(app, /return compareRequestOptionStrength\(a, b\)/, "request picker should not fall back to alphabetical option ordering")
  assert.match(app, /return String\(item\.medium_url \|\| item\.thumb_url \|\| ""\)\.trim\(\)/, "request picker should prefer the less aggressively cropped medium rendition")
