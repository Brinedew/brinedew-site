...
class FakeStatement {
  ...
  async all() {
    ...
    if (
      ...
      return {
        results: rows.slice(0, Math.max(0, Number(limit || 0) || 0)).map((row) => ({ ...row })),
      }
    }
    if (
      this.sql.includes("FROM icono_sync_finalization_jobs") &&
      this.sql.includes("WHERE status IN (?, ?)")
    ) {
      ...
      try {
        ...
        scopedSymbols = Array.isArray(parsed)
          ? parsed
              .map((item) =>
                String(item || "")
                  .trim()
      ...
      const rows = [...this.db.jobs.values()]
        ...
        .filter(
          (row) => !row.next_attempt_at || String(row.next_attempt_at) <= String(nowIso || ""),
        )
        .filter(
          (row) =>
            Number(scopedEnabled || 0) <= 0 ||
            scopedSymbols.includes(
              String(row.gene_symbol || "")
                .trim()
                .toUpperCase(),
            ),
        ...
        .sort((left, right) => {
          ...
          const requestedDelta = String(left.requested_at || "").localeCompare(
            String(right.requested_at || ""),
          )
    ...
    if (
      ...
      if (this.sql.includes("SELECT gene_symbol") && this.sql.includes("AND phase = ?")) {
        ...
        const rows = [...this.db.jobs.values()]
          ...
          .sort((left, right) => {
            const nextAttemptDelta = String(left.next_attempt_at || "").localeCompare(
              String(right.next_attempt_at || ""),
            )
      ...
      const rows = [...this.db.jobs.values()]
        ...
        .sort((left, right) => {
          ...
          if (leftPendingFinalize !== rightPendingFinalize)
            return rightPendingFinalize - leftPendingFinalize
          const nextAttemptDelta = String(left.next_attempt_at || "").localeCompare(
            String(right.next_attempt_at || ""),
          )
          if (nextAttemptDelta !== 0) return nextAttemptDelta
  ...
  async first() {
    ...
    if (
      this.sql.includes("SELECT MAX(completed_at) AS completed_at") &&
      this.sql.includes("FROM icono_sync_finalization_jobs")
    ) {
    ...
    if (
      ...
      if (this.sql.includes("phase NOT IN (?, ?)") && this.sql.includes("status <> ?")) {
        const [excludedStatus, excludedPhaseA, excludedPhaseB, scopedEnabled, scopedSymbolsJson] =
          this.args
        const scopedSymbols = parseScopedSymbols(scopedEnabled, scopedSymbolsJson)
        return {
          count: [...this.db.jobs.values()].filter(
    ...
    store: new Map([
      ...
      [
        ...
        JSON.stringify({
          ...
          shards: [
            {
              key: "iconoplasm:card-catalog-shard:test-baseline",
              index: 0,
              card_count: 1,
...
async function observeHealthyGovernorForTest(governor) {
  const response = await governor.fetch(
    new Request("https://iconoplasm-sync-governor/release", {
      ...
      body: JSON.stringify({ public_health: "healthy", processed: 0, failed: 0, retrying: 0 }),
    }),
  )
...
test("admin finalization enqueue stores normalized durable job rows", async () => {
  ...
  assert.deepEqual(JSON.parse(String(stored?.keep_assets_json || "[]")), [
    { symbol: "TP53", asset_sha256: "a".repeat(64) },
  ])
  assert.deepEqual(JSON.parse(String(stored?.legacy_assets_json || "[]")), [
      ...
      get() {
        return {
          fetch: async (request) => {
            ...
            if (url.pathname === "/permit") {
              return new Response(
                JSON.stringify({ ok: true, granted: 1, lease_id: "lease-test" }),
                {
                  headers: { "Content-Type": "application/json" },
                },
              )
