...
class FakeStatement {
  ...
  async all() {
    ...
    if (this.sql.includes("FROM icono_sync_finalization_jobs") && this.sql.includes("WHERE status IN (?, ?)")) {
      const [queuedStatus, retryingStatus, excludedPhase, nowIso, limit] = this.args
  ...
  async first() {
    ...
    if (this.sql.includes("SELECT COUNT(*) AS count") && this.sql.includes("FROM icono_sync_finalization_jobs")) {
      ...
      if (this.sql.includes("WHERE phase = ? AND status <> ?")) {
        const [phase, excludedStatus] = this.args
        return { count: [...this.db.jobs.values()].filter((row) => row.phase === phase && row.status !== excludedStatus).length }
      }
      if (this.sql.includes("WHERE status <> ?")) {
        const [excludedStatus] = this.args
        return { count: [...this.db.jobs.values()].filter((row) => row.status !== excludedStatus).length }
      }
    }
    return null
  }
