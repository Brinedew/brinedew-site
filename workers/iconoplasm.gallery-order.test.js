...
class FakeStatement {
  ...
  async all() {
    if (
      ...
      const metricKey = this.sql.includes("CASE WHEN ge.weight_kg") ? "weight_kg" : "age_years"
      const descending = this.sql.includes(
        `${metricKey === "age_years" ? "ge.age_years" : "ge.weight_kg"} DESC`,
      )
      const invalidatesNonPositive = this.sql.includes(
        `${metricKey === "age_years" ? "ge.age_years" : "ge.weight_kg"} <= 0`,
      )
      ...
      const sorted = this.db.rows.slice().sort((left, right) => {
        ...
        const leftInvalid =
          !Number.isFinite(leftMetric) || (invalidatesNonPositive && leftMetric <= 0)
        const rightInvalid =
          !Number.isFinite(rightMetric) || (invalidatesNonPositive && rightMetric <= 0)
