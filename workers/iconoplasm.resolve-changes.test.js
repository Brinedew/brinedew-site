...
class FakeChangesStatement {
  ...
  async all() {
    ...
    if (this.sql.includes("FROM icono_publish_state") && this.sql.includes("current_asset_sha256")) {
      return {
        results: [
          {
            symbol: "PRL",
...
test("public resolve route works through THE_ONLY_ALLOWED_STATEFUL_WORKER_DO_NOT_DUPLICATE", async () => {
  ...
  assert.deepEqual(
    payload?.results?.map((item) => ({ requested: item.requested, canonical_symbol: item.canonical_symbol, matched_by: item.matched_by })),
    [
