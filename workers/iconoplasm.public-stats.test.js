...
test("admin asset summary refresh writes public stats projection to KV", async () => {
  ...
  const response =
    await handleIconoplasmRequestInsideTheOnlyAllowedInternalStatefulWorkerDoNotDuplicate(
      new Request("https://iconoplasm.brinedew.bio/api/iconoplasm/admin/assets/summary?refresh=1", {
...
test("admin asset summary is read-only by default and exposes public versus ledger scope", async () => {
  const kv = new FakeKv({
    "iconoplasm:public-stats:v1": JSON.stringify({
      schema_version: "iconoplasm.publicStats.v1",
      gene_count: 19023,
      canonical_blot_count: 19023,
      generated_candidate_blot_count: 44836,
      auditable_candidate_blot_count: 44436,
      storage_verified_candidate_blot_count: 27367,
      storage_audit_coverage_percent: 61.3,
      storage_audit_complete: false,
      updated_at: "2026-05-21T06:04:25.433Z",
    }),
  })
  const db = {
    prepare(sql) {
      ...
      return {
        ...
        async first() {
          if (text.includes("FROM icono_website_truth_summary")) {
            return {
              candidate_assets: 45041,
              stale_assets: 0,
              legacy_assets: 0,
              published_live_portraits: 19090,
              audited_assets: 27367,
              verified_renderable_images: 27367,
              storage_audit_coverage_percent: 61.3,
              storage_incomplete_assets: 0,
              broken_live_images: 0,
              renderable_live_confirmed: 19179,
              unverified_live_portraits: 0,
              renderable_live_exact_known: 0,
              storage_queue_backlog_assets: 17069,
              storage_queue_seeded_complete: 1,
              storage_audit_status_note: "Persisted proof summary.",
              updated_at: "2026-05-21 06:04:25",
            }
          }
  ...
  assert.equal(response.status, 200)
  assert.equal(kv.puts.length, 0, "passive summary reads must not write public stats KV")
  const payload = await response.json()
  assert.equal(payload.refreshed, false)
  assert.equal(payload.public_scope.catalog_genes, 19023)
  assert.equal(payload.public_scope.catalog_published_live_portraits, 19023)
  assert.equal(payload.public_scope.catalog_candidate_assets, 44836)
  assert.equal(payload.ledger_scope.published_live_portraits, 19090)
  assert.equal(payload.ledger_scope.candidate_assets, 45041)
  assert.equal(payload.ledger_scope.non_catalog_live_portraits, 67)
  assert.equal(payload.ledger_scope.non_catalog_candidate_assets, 205)
})

test("admin asset summary refresh skips KV write when public stats did not materially change", async () => {
  const existingPublicStats = {
    schema_version: "iconoplasm.publicStats.v1",
    gene_count: 19023,
    canonical_blot_count: 19023,
    generated_candidate_blot_count: 39481,
    auditable_candidate_blot_count: 39112,
    storage_verified_candidate_blot_count: 21889,
    storage_audit_coverage_percent: 55.9,
    storage_audit_complete: false,
    updated_at: "2026-05-03 13:02:48",
  ...
  const kv = new FakeKv({
    ...
    "iconoplasm:public-stats:v1": JSON.stringify(existingPublicStats),
  })
  const db = {
    prepare(sql) {
      const text = String(sql || "")
  ...
  assert.equal(response.status, 200)
  assert.deepEqual(kv.puts, [], "same public counts must not spend another KV write")
})
