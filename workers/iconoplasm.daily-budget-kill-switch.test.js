...
test("daily budget durable object rebuilds legacy attribution schema before reporting", async () => {
  ...
  const rebuildInsert = executedSql.find((sql) => sql.includes("INSERT INTO daily_budget_usage_attribution_v2_do_not_delete")) || ""

  assert.equal(
    executedSql.some((sql) => sql.includes("CREATE TABLE IF NOT EXISTS daily_budget_usage_attribution_v2_do_not_delete")),
    true,
  )
  assert.equal(rebuildInsert.includes("day_key AS cycle_key"), true)
  assert.equal(rebuildInsert.includes("0 AS request_count"), true)
  assert.equal(executedSql.some((sql) => sql.includes("DROP TABLE daily_budget_usage_attribution")), true)
  assert.equal(
    executedSql.some((sql) =>
      sql.includes("ALTER TABLE daily_budget_usage_attribution_v2_do_not_delete RENAME TO daily_budget_usage_attribution"),
    ),
    true,
  )
})
...
test("daily budget report survives constructor schema writes after DO free-tier write exhaustion", async () => {
  class FreeTierExhaustedButReadableSql {
    ...
    exec(sql, ...args) {
      ...
      if (text.includes("FROM daily_budget_usage_attribution") && text.includes("WHERE day_key = ?")) {
        return { toArray: () => this.attributionRows }
      }
      if (text.includes("FROM daily_budget_usage_attribution") && text.includes("WHERE cycle_key = ?")) {
        return { toArray: () => this.attributionRows }
      }
      if (text.includes("FROM daily_budget_usage\n         WHERE cycle_key = ?")) {
        return { toArray: () => [this.dayRow] }
      ...
      "https://the-only-allowed-internal-stateful-worker-do-not-duplicate/api/iconoplasm/admin/catalog/upsert",
      {
        ...
        body: JSON.stringify({
          ...
          items: [
            ...
            { gene_symbol: "BRCA1", full_name: "Breast cancer type 1 susceptibility protein", tmh: false, aliases_json: [] },
            { gene_symbol: "MYC", full_name: "MYC proto-oncogene", tmh: false, aliases_json: [] },
            { gene_symbol: "PTEN", full_name: "Phosphatase and tensin homolog", tmh: false, aliases_json: [] },
          ],
        }),
      },
    ),
