...
function makeEvidenceDb({ observationResults = [], failureResults = [] } = {}) {
  return {
    prepare(sql) {
      ...
      if (sql.includes("SELECT") && sql.includes("game_session_write_failure_samples_do_not_delete")) {
        return {
          bind() {
            return {
...
test("game session evidence summarizes successes before the first failure and keeps failure samples", async () => {
  ...
  assert.equal(evidence.failure_fingerprints[0].error_fingerprint, "Exceeded allowed rows written in Durable Objects free tier.")
