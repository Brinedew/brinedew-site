...
// src/archive.ts
function archiveStatus(page) {
  const draft = page.frontmatter?.draft;
  return draft === true || draft === "true" ? "draft" : "published";
}
function archiveDate(page) {
  if (!page.defaultDateType) return void 0;
  return page.dates?.[page.defaultDateType];
}
var byArchiveDate = (left, right) => {
...
function partitionArchive(pages, sort = byArchiveDate) {
...
function PageList({ fileData, allFiles, pages, limit, sort }) {
  const ordered = [...pages ?? allFiles].sort(sort ?? byArchiveDate);
  const list = limit ? ordered.slice(0, limit) : ordered;
  return /* @__PURE__ */ u2("ul", { class: "tag-page-list", children: list.map((page) => {
    ...
    return /* @__PURE__ */ u2("li", { children: /* @__PURE__ */ u2(
      "a",
      {
        ...
        children: [
          /* @__PURE__ */ u2("span", { children: page.frontmatter?.title }),
          date && /* @__PURE__ */ u2("time", { datetime: date.toISOString(), children: date.getFullYear() })
        ]
      }
    ) });
  }) });
...
function formatArchiveDate(date, locale) {
...
function ArchiveList({ pages, currentSlug, locale, emptyMessage }) {
  if (pages.length === 0) return /* @__PURE__ */ u2("p", { class: "archive-index__empty", children: emptyMessage });
  return /* @__PURE__ */ u2("ol", { class: "archive-index__list", children: pages.map((page) => {
    const date = archiveDate(page);
    return /* @__PURE__ */ u2("li", { class: "archive-index__entry", children: /* @__PURE__ */ u2(
      "a",
      {
        class: "archive-index__link internal internal-link",
        href: resolveRelative(currentSlug, page.slug),
        children: [
          /* @__PURE__ */ u2("span", { class: "archive-index__title", children: page.frontmatter?.title }),
          date && /* @__PURE__ */ u2("time", { class: "archive-index__date", datetime: date.toISOString(), children: formatArchiveDate(date, locale) })
        ]
      }
    ) });
  }) });
        ...
        children: [
          /* @__PURE__ */ u2("div", { class: "archive-index__section-heading", children: [
            ...
            /* @__PURE__ */ u2("p", { children: [
              published.length,
              " finished ",
              published.length === 1 ? "piece" : "pieces"
            ] })
          ] }),
          /* @__PURE__ */ u2(
            ArchiveList,
...
var editorialArchiveTags = /* @__PURE__ */ new Set(["content/post", "content/wiki"]);
function isListed(file) {
  return file.unlisted !== true;
}
var TagContent_default = ((options) => {
  const TagContent = (props) => {
    ...
    const pagesFor = (candidate) => allFiles.filter(isListed).filter(
      (file) => (file.frontmatter?.tags ?? []).flatMap(getAllSegmentPrefixes).includes(candidate)
    );
    if (editorialArchiveTags.has(tag)) {
      return /* @__PURE__ */ u2("div", { class: "popover-hint archive-index-shell", children: /* @__PURE__ */ u2(
        ArchiveIndex,
        {
          tag,
    ...
    if (tag === "/") {
      const tags = [
        ...new Set(
          allFiles.filter(isListed).flatMap((file) => file.frontmatter?.tags ?? []).flatMap(getAllSegmentPrefixes)
        )
      ].sort((left, right) => left.localeCompare(right));
      return /* @__PURE__ */ u2("main", { class: "tag-index", children: [
        /* @__PURE__ */ u2("h1", { children: fileData.frontmatter?.title ?? "Tags" }),
        /* @__PURE__ */ u2("ul", { children: tags.map((item) => /* @__PURE__ */ u2("li", { children: /* @__PURE__ */ u2("a", { class: "internal tag-link", href: `./${item}`, children: item }) })) })
      ] });
    }
    const pages = pagesFor(tag);
    const listProps = { ...props, pages, sort: options?.sort };
    return /* @__PURE__ */ u2("main", { class: "tag-page-listing popover-hint", children: [
      /* @__PURE__ */ u2("header", { children: [
        /* @__PURE__ */ u2("h1", { children: fileData.frontmatter?.title ?? tag }),
        /* @__PURE__ */ u2("p", { children: pages.length === 1 ? "1 item" : `${pages.length} items` })
      ] }),
      /* @__PURE__ */ u2(PageList, { ...listProps })
    ] });
  };
  TagContent.css = tagPage_default;
  return TagContent;
});
