;(function (root) {
  ...
  function compareCandidateLists(left, right) {
    ...
    for (let i = 0; i < Math.min(leftList.length, rightList.length); i += 1) {
      const byStart = (Number(rightList[i] && rightList[i].index) || 0) - (Number(leftList[i] && leftList[i].index) || 0)
      if (byStart !== 0) return byStart
      const byLength = (Number(leftList[i] && leftList[i].length) || 0) - (Number(rightList[i] && rightList[i].length) || 0)
  ...
  function selectBestRunCandidates(candidates) {
    ...
    function solve(index) {
      ...
      const currentEnd = (Number(current && current.index) || 0) + (Number(current && current.length) || 0)
