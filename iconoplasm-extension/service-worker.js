...
function normalizePublishedManifest(rawManifest) {
  ...
  const currentHash = String(manifest.build_version || manifest.current_hash || manifest.catalog_hash || "").trim()
  const catalogHash = String(manifest.catalog_hash || "").trim()
  const filename = String(manifest.filename || (catalogHash ? `catalog.${catalogHash}.json` : "")).trim()
  ...
  const minExtensionVersion = String(
    manifest.min_extension_version || manifest.minimum_extension_version || currentExtensionVersion(),
  ).trim()
  if (!currentHash || (!filename && !artifactUrl) || !portraitBaseUrl || !Number.isFinite(schemaVersion)) {
    return null
  }
  return {
...
async function clearContractError() {
  await chrome.storage.local.remove(["iconoplasm_contract_error", "iconoplasm_min_extension_version"])
}
...
async function ensureFreshGeneData() {
  ...
  const needsRefresh = hasContractError || needsArtifactRebuild || isStaleFetch(stored.iconoplasm_last_fetch)
...
async function fetchPortraitDataUrl(url) {
  ...
  try {
    ...
    rememberPortraitDataUrlError(normalizedUrl, err && err.message ? err.message : String(err || "fetch_error"))
    return ""
  }
}
...
async function warmPortraitDataUrls(urls) {
  ...
  const results = await Promise.all(normalized.map((url) => fetchPortraitDataUrl(url).catch(() => "")))
  ...
  try {
    ...
    if (!artifact || !Array.isArray(artifact.genes)) {
      await invalidateStoredPublishedSnapshot({
        code: CONTRACT_ERROR_INVALID_MANIFEST,
        minExtensionVersion: manifest.min_extension_version,
        message: "Published catalog artifact is missing the genes array required by the extension runtime.",
      })
