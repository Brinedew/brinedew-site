...
Iconoplasm scans web pages you visit for human gene symbols (like TP53, BRCA1). When it finds them, it highlights the text and shows hover cards with the gene's assigned color identity and blot. It also tracks which genes you've encountered (your "discovery" count).
...
When the extension finds gene symbols on a page, it sends those symbols — and only those symbols — to `iconoplasm.brinedew.bio` to fetch color data and blots. We do not send the URL you're visiting, the page content, or any other browsing data.
