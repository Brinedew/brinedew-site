...
<script>
(function() {
    // Configuration - fallback to localhost if tunnel fails
    const TUNNEL_URL = 'https://api2.brinedew.com';
    const LOCAL_URL = 'http://localhost:5000';
    let API_BASE_URL = TUNNEL_URL;
    ...
    async function startFlaskServer() {
        try {
            // Try to start Flask server via API call to wake it up
            ...
            console.log('Could not start Flask server:', error.message);
        }
    }

    async function checkServerStatus() {
        // Try tunnel first, then fallback to localhost
        const urls = API_BASE_URL === TUNNEL_URL ? [TUNNEL_URL, LOCAL_URL] : [LOCAL_URL];
        ...
        for (const url of urls) {
            try {
                ...
                // Success - update API_BASE_URL and status
                API_BASE_URL = url;
                updateServerStatus(data.status, data.message, true);
                return;
                
            } catch (error) {
                console.log(`API unavailable at ${url}:`, error.message);
                continue;
            }
        }
