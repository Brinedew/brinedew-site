 ...
 * ------------------------------------------------
 * 1. Source of truth: Datasets/GeneGuessr/ pipeline
...
;(function () {
  ...
  function safeJsonParse(raw, options = {}) {
    const { label = "value", storageKey, storageArea = "local", fallback = null } = options
  ...
  function getStructureMetaLabel(structure) {
    ...
    if (structure.primary_source === "pdb" && structure.structure_id) {
      const resolution =
        structure.pdb && typeof structure.pdb.resolution === "number"
    ...
    if (structure.pdb && structure.pdb.id) {
      const resolution =
        typeof structure.pdb.resolution === "number"
          ? `${structure.pdb.resolution.toFixed(2)} A`
  ...
  function formatSwissLabel(model) {
    ...
    const coveragePart =
      typeof model.coverage === "number" ? `${Math.round(model.coverage * 100)}%` : ""
    const qmeanPart = typeof model.qmean === "number" ? `QMEAN ${model.qmean.toFixed(2)}` : ""
  ...
  function resolveStructureRepresentation(structure, proteinLength) {
    ...
    const structureId =
      ...
      (structure.pdb && structure.pdb.id) ||
      (swissModel && (swissModel.model_id || swissModel.template || swissModel.pdb_id)) ||
      (structure.alphafold && structure.alphafold.id) ||
      ""
    const base = {
      coverage,
      structureId,
  ...
  function getStructureSourceMetadataFromRepresentation(representation) {
    ...
    if (representation.source === "swissmodel" && representation.swissModel) {
      const url =
        representation.swissModel.model_url || representation.swissModel.coordinates_url || null
  ...
  function getStructureSourceInfo(protein) {
    ...
    return {
      source: representation.source,
      shortLabel: representation.source,
      longLabel: representation.source,
      representation,
    }
  }
  ...
  function normalizeSwissModel(raw, proteinLength) {
    ...
    normalized.coverage =
      typeof raw.coverage === "number" ? raw.coverage : computeSwissCoverage(raw, proteinLength)
    normalized.qmean = typeof raw.qmean === "number" ? raw.qmean : extractSwissQuality(raw)
    const chainCandidates =
      Array.isArray(raw.chain_ids) && raw.chain_ids.length
        ? raw.chain_ids
        : Array.isArray(raw.chains) && raw.chains.length
          ? raw.chains.map((c) => c && c.id).filter(Boolean)
          : raw.chain_id
            ? [raw.chain_id]
            : []
    normalized.chain_ids = chainCandidates
    normalized.uniprot_start = toFiniteNumber(
      raw.uniprot_start ?? raw.uniprot_from ?? raw.start ?? raw.from,
    )
  ...
  function computeSwissCoverage(model, proteinLength) {
    ...
    const start = toFiniteNumber(
      model.uniprot_start ?? model.uniprot_from ?? model.start ?? model.from,
    )
  ...
  function extractSwissQuality(model) {
    ...
    const candidates = [
      ...
      model.qmean &&
        (model.qmean.qmeanDisCo_global ??
          model.qmean.qmean_dis_co_global ??
          model.qmean.qmean4_norm_score ??
          model.qmean.avg_local_score),
    ]
  ...
  async function ensureStructureTokenForProtein(uniprot) {
    ...
    try {
      ...
      const resp = await fetch(
        `${API_BASE}/api/structure-token?uniprot=${encodeURIComponent(key)}`,
        {
          credentials: "include",
        },
      )
      console.log(
        `[TIMING] token for ${key} | API responded | ${(performance.now() - t0).toFixed(0)}ms`,
      )
      if (!resp.ok) {
        if (resp.status === 404) {
  ...
  function renderStructureViewer(protein, viewerId, options = {}) {
    ...
    const shortLabel =
      structureInfo?.sourceLabel || structureSource?.shortLabel || "Source unavailable"
    ...
    const displayLabel = linkable ? (hasRealData ? longLabel : "Loading...") : shortLabel
  ...
  function setupStructureInteractions() {
    ...
    for (const guess of guessesToLoad) {
      ...
      if (container && isExpanded && !renderedViewers.has(viewerId)) {
        loadStructureViewerInContainer(container, { uniprot: guess.uniprot }).catch((err) => {
          console.error(
            `Geneguessr: failed to load structure viewer for guess ${guess.guessId}`,
            err,
          )
        })
        loadedGuessViewers++
    ...
    console.log(
      `[GeneGuessr] setupStructureInteractions: loaded ${loadedGuessViewers} guess viewers (limit: ${MAX_GUESS_VIEWERS_ON_LOAD})`,
    )
  }
  ...
  function getAccentColorRgb() {
    return (
      resolveCssColorValue("var(--accent)") ||
      hexToRgb(ACCENT_COLOR_HEX) || { r: 27, g: 114, b: 105 }
    )
  }
  ...
  async function applyChainColoring(viewer, representation, container) {
    if (
      ...
      !representation ||
      (representation.source !== "pdb" && representation.source !== "swissmodel")
    ) {
      return
    }
  ...
  function buildChainHighlightData(representation, accentRgb) {
    ...
    if (representation.source === "pdb") {
      segments =
        representation.chains && representation.chains.length > 0
  ...
  function updateCallouts(viewerId) {
    ...
    overlay.querySelectorAll(".pg-chain-callout[data-chain-id]").forEach((el) => {
      ...
      // Clamp to viewport
      const pad = 10,
        ew = el.offsetWidth || 80,
        eh = el.offsetHeight || 24
  ...
  function renderChainLabelCallouts(container, chainLabels, totalChainCount) {
    ...
    display.forEach((entry) => {
      ...
      el.innerHTML =
        `<span class="pg-chain-label-gene">${escapeAttribute(gene)}</span>` +
  ...
  async function createPerChainComponents(viewerId, chainLabels) {
    ...
    console.log(
      "[Geneguessr] createPerChainComponents: creating components for chains:",
    ...
    try {
      ...
      for (const { chainId, isTarget, labelName } of allChains) {
        try {
          const selections =
            typeof viewer.getSelections === "function"
          ...
          if (
            !selection ||
            !selection.loci ||
            !selection.loci.elements ||
            selection.loci.elements.length === 0
          ) {
  ...
  async function applyChainVisibility(viewerId, showComplex) {
    ...
    if (!chainLabels || countTotalChains(chainLabels) <= 1) {
      console.log(
        "[Geneguessr] applyChainVisibility: skipping (single chain or no labels)",
        viewerId,
      )
      return
    }
    ...
    const nonTargetChains = new Set(
      chainLabels.filter((l) => !l.is_target).flatMap((l) => l.chains),
    )
    ...
    try {
      ...
      console.log(
        "[Geneguessr] applyChainVisibility completed with per-chain refs (showComplex=" +
          showComplex +
          ")",
      )
    } catch (err) {
  ...
  async function loadStructureViewerInContainer(container, protein) {
    if (!container || !protein) {
      console.warn("[Geneguessr] loadStructureViewerInContainer: missing container or protein", {
        container: !!container,
        protein: !!protein,
      })
      return
    ...
    const t0 = performance.now()
    const timing = (label) =>
      console.log(`[TIMING] ${containerId} | ${label} | ${(performance.now() - t0).toFixed(0)}ms`)
    timing("start")
    ...
    console.debug(
      "[Geneguessr] loadStructureViewerInContainer: initial structureInfo",
      containerId,
      structureInfo,
    )
    if (!structureInfo || !structureInfo.url) {
      ...
      console.debug(
        "[Geneguessr] loadStructureViewerInContainer: resolved structureInfo",
        containerId,
        structureInfo,
      )
    } else {
    ...
    if (!structureInfo || !structureInfo.url) {
      console.warn(
        "[Geneguessr] loadStructureViewerInContainer: no structureInfo or url",
        containerId,
        structureInfo,
      )
      if (errorEl) {
    ...
    console.debug(
      "[Geneguessr] loadStructureViewerInContainer: structureUrl resolved",
      containerId,
      structureUrl,
    )
    if (!structureUrl) {
      console.warn(
        "[Geneguessr] loadStructureViewerInContainer: no structureUrl",
        containerId,
        structureInfo,
      )
      if (errorEl) {
  ...
  async function loadStructureViewer() {
    ...
    const container =
      document.getElementById("pg-clue-structure") ||
      document.getElementById("pg-solution-card-structure")
  ...
  function buildPracticeQuery(options = {}) {
    const practice =
      typeof options.practice === "boolean" ? options.practice : Boolean(gameState.practiceMode)
    if (!practice) {
      return ""
    }
  ...
  async function fetchGameBootstrap(options = {}) {
    const response = await fetch(
      `${API_BASE}/api/game/bootstrap${buildPracticeQuery({
  ...
  async function submitGuessRequest(uniprot) {
    ...
    console.log(
      `[TIMING] guess-api-call (network + worker compute) | ${Math.round(performance.now() - t0)}ms`,
    )
    if (!response.ok) {
      const error = await response.json().catch(() => ({}))
  ...
  function hydrateStateFromPayload(payload) {
    ...
    targetProtein = payload.clueTarget
      ? cacheEnrichedProtein(payload.clueTarget) || payload.clueTarget
      : null
    targetReveal = payload.targetReveal
      ? cacheEnrichedProtein(payload.targetReveal) || payload.targetReveal
      : null
    ...
    if (payload.targetStructureToken && payload.targetStructureToken.url) {
      ...
      console.log(
        `[TIMING] pre-zero-time (navigation → first timing log) | ${Math.round(preZeroTime)}ms`,
      )
      ...
      targetStructureInfo = {
        ...
        displayLabel:
          payload.targetStructureToken.displayLabel ||
          payload.targetStructureToken.sourceLabel ||
          "Source unavailable",
    ...
    console.log(
      `[B-206 DEBUG] Hydrated state - date: ${payload.status.date}, guesses: ${guessEntries.length}`,
    )
  ...
  function mergeProteinRecords(base, overrides) {
    ...
    return {
      ...
      reactome_pathways: Array.isArray(overrides.reactome_pathways)
        ? overrides.reactome_pathways
        : base.reactome_pathways,
  ...
  async function bootstrapGame(options = {}) {
    ...
    console.log(
      `[TIMING] pre-bootstrap (navigation → bootstrap call start) | ${Math.round(preBootstrapTime)}ms`,
    )
  ...
  async function pickTodaysProtein(eligibleIds, salt) {
    ...
    if (!filteredIds.length) {
      console.warn(
        "[Geneguessr] AlphaFold-only question pool detected, falling back to full eligible list.",
      )
    }
  ...
  function filterEligibleQuestionIds(eligibleIds) {
    ...
    if (skipped) {
      console.info(
        `[Geneguessr] Skipped ${skipped} AlphaFold-only proteins from today's question pool.`,
      )
    }
    return filtered
  }
  ...
  function renderLockedHintPlaceholder(item, entryClass) {
    ...
    const maskLength =
      Number(item?.maskLength) || placeholder.length || LOCKED_HINT_PLACEHOLDER.length
    ...
    const lockedClicked =
      locked &&
      Array.isArray(gameState.lockedHintClicks) &&
      gameState.lockedHintClicks.includes(item.id)
  ...
  function renderClueCard(gameOver = false) {
    if (gameOver) {
      const solutionTarget = targetReveal || targetProtein
      const revealStructureInfo = solutionTarget?.uniprot
        ? getStructureInfoForProtein(solutionTarget.uniprot)
        : getTargetStructureInfo()
      const solutionStructureSource = solutionTarget?.uniprot
        ? { type: "uniprot", id: solutionTarget.uniprot }
        : { type: "target" }
      const latestGuessEntry =
        Array.isArray(gameState.guesses) && gameState.guesses.length
      ...
      .map((item) => {
        ...
        if (showSpoilers && item.id) {
          ...
          if ((revealed || forceReveal) && text) {
            const cls = isMatched
              ? "pg-section-entry pg-revealed-text matched-highlight"
              : "pg-section-entry pg-revealed-text"
            return `<span class="${cls}">${escapeHtml(text)}</span>`
          }
          return renderLockedHintPlaceholder(
            item,
            isMatched ? "pg-section-entry matched-highlight" : "pg-section-entry",
          )
        }
  ...
  function renderClueSectionsIntoDom(gameOver = false) {
    ...
    if (viewerAlreadyLoaded && (dateChanged || sessionChanged || transitioningToGameOver)) {
      console.log(
        `[B-206 DEBUG] Disposing clue viewer - dateChanged: ${dateChanged}, sessionChanged: ${sessionChanged}`,
      )
      disposeViewer("pg-clue-structure")
    }
    if (gameOver) {
  ...
  function renderInputSection(gameOver) {
    ...
    if (
      ...
      typeof window.GeneGuessrTutorial.open === "function"
    ) {
  ...
  function buildFeedbackCardMarkup(protein, options = {}) {
    ...
    const sectionMarkup = sections
      .map((section) =>
        renderFeedbackSection(section, score, matchedHintMap[section.id] || [], {
          highlightMatches,
        }),
      )
      .join("")
  ...
  function renderCollapsibleFeedback(guessEntry, isLatest) {
    ...
    const isLatestGuess =
      typeof guessEntry.isLatest === "boolean" ? guessEntry.isLatest : Boolean(isLatest)
    ...
    return buildFeedbackCardMarkup(
      { uniprot: guessEntry.uniprot },
      {
        score,
        cardId,
        collapsible: true,
    ...
    if (structureContainer) {
      ...
      if (!newExpanded) {
        ...
        if (!renderedViewers.has(viewerId)) {
          ...
          if (guess) {
            loadStructureViewerInContainer(structureContainer, { uniprot: guess.uniprot }).catch(
              (err) => {
  ...
  async function activateSpoiler(redaction) {
    ...
    try {
      ...
      if (payload.revealedHint && payload.revealedHint.text) {
        ...
        console.log(
          `[HINT REVEAL] DOM update: ${Math.round(domTime)}ms | Total: ${Math.round(totalTime)}ms (surgical path)`,
        )
      } else {
        // Fallback path for backward compatibility (slow)
        console.warn(
          `[HINT REVEAL] Falling back to full render() - API did not return revealedHint! Response:`,
          payload,
        )
        hydrateStateFromPayload(payload)
        updateHintDisplays()
  ...
  async function searchProteins(query) {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/proteins?query=${encodeURIComponent(cleanedQuery)}&limit=${SEARCH_MAX_RESULTS}${excludeParam}`,
        {
          credentials: "include",
      ...
      const normalizedResults = Array.isArray(payload)
        ? payload.map((protein) => rememberProteinRecord(protein)).filter(Boolean)
        : []
  ...
  async function handleGuessPayload(uniprot, payload, guessStartTime) {
    ...
    if (!newGuess?.sections) {
      ...
      try {
        ...
        console.log(
          `[TIMING] guess-submit | bootstrap fallback | ${Math.round(performance.now() - bootstrapT0)}ms`,
        )
    ...
    if (guessStartTime != null) {
      console.log(
        `[TIMING] guess-submit | render() start | ${Math.round(performance.now() - guessStartTime)}ms`,
      )
    }
    render()
    if (guessStartTime != null) {
      console.log(
        `[TIMING] guess-submit | render() done | ${Math.round(performance.now() - guessStartTime)}ms`,
      )
    }
  ...
  async function submitGuess() {
    ...
    if (!uniprot && inputEl && inputEl.value.trim()) {
      ...
      try {
        ...
        const exactMatch = matches.find((p) => p.hgnc.toUpperCase() === query.toUpperCase())
...
https://brinedew.bio/apps/geneguessr/`
  ...
  function getCurrentSidebarStats() {
    if (currentUser && sidebarStatsSnapshot && typeof sidebarStatsSnapshot === "object") {
  ...
  function renderSidebarLeaderboardRows(entries, isLoading = false) {
    if (isLoading) {
      return '<div class="pg-sidebar-empty">Loading leaderboard...</div>'
    }
    if (!Array.isArray(entries) || entries.length === 0) {
      return '<div class="pg-sidebar-empty">No public streaks yet.</div>'
    }
    return entries
      ...
      .map((entry, idx) => {
        ...
        return `
          <div class="pg-leaderboard-row">
            <span class="pg-leaderboard-rank">#${rank}</span>
            ${avatarMarkup}
            <span class="pg-leaderboard-name" title="${escapeHtml(username)}">${escapeHtml(username)}</span>
            <span class="pg-leaderboard-streak">${streak}</span>
          </div>
        `
      })
      .join("")
  ...
  async function refreshSidebarStatsSnapshot() {
    if (!currentUser) {
      sidebarStatsSnapshot = null
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.authenticated) {
          ...
          setLeaderboardConsentEnabled(Boolean(data?.user?.leaderboard_opt_in))
          return true
        }
      }
    } catch (err) {
  ...
  function injectSidebarStats() {
    ...
    `
    const consentSection = currentUser
      ? `
      <div class="pg-sidebar-section pg-sidebar-consent-section">
        <div class="pg-sidebar-label">Leaderboard</div>
    ...
    `
      ...
      <div class="pg-sidebar-section pg-sidebar-consent-section">
        ...
        <div id="pg-auth-visibility-status" class="pg-auth-consent-help">${loggedOutVisibilityText}</div>
      </div>
    `
    ...
    sidebarStats.innerHTML = `
      ...
      ${consentSection}
      <div class="pg-sidebar-section">
        <div class="pg-sidebar-label">Top Streaks</div>
  ...
  async function resolvePracticeListFromTextarea() {
    ...
    try {
      ...
      const practiceList = {
        ...
        recognizedUnplayable: Array.isArray(payload.recognizedUnplayable)
  ...
  window.geneguessrUpdateLeaderboardVisibility = async function (enabled) {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/stats/leaderboard-visibility`, {
        ...
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ optIn: desired }),
      })
      const payload = await response.json().catch(() => ({}))
      if (!response.ok) {
        throw new Error(payload?.error || "Failed to update leaderboard visibility")
      }
  ...
  async function init() {
    ...
    if (
      !tutorialBootRequested &&
      window.GeneGuessrTutorial &&
      typeof window.GeneGuessrTutorial.boot === "function"
    ) {
  ...
  function attachSourcesToggle() {
    ...
    const card =
      document.getElementById("sources-card") || document.getElementById("attribution-card")
    if (!card) return
    ...
    toggle.addEventListener("click", function () {
      const content =
        document.getElementById("sources-content") || document.getElementById("attribution-content")
      const chevron = card.querySelector(".pg-collapse-chevron")
      const expanded = card.dataset.expanded === "true"
      const next = !expanded
  ...
  function buildMaskCharacters(length) {
    const cap = Math.min(
      Math.max(Number(length) || LOCKED_HINT_PLACEHOLDER.length, LOCKED_HINT_PLACEHOLDER.length),
      64,
    )
    return "█".repeat(cap)
  }
