...
function styleColumnTemplate(column, index) {
  ...
  var columnClass = "icono-label-style-column icono-label-style-column--" + (index === 0 ? "left" : "right");
  return b2`<div class=${columnClass}>${safeColumn.map(stylePairTemplate)}</div>`;
}
function styleStackTemplate(model) {
  var columns = Array.isArray(model.stylePairColumns) ? model.stylePairColumns : splitStylePairColumns(model.stylePairs);
  var stackClass = "icono-label-style-stack";
  if (columns.length > 1) stackClass += " icono-label-style-stack--two-column";
  return b2`<div class=${stackClass}>${columns.map(styleColumnTemplate)}</div>`;
}
...
function sexNoteTemplate(sexNote, selectedCategory) {
  ...
  return b2`<div role="note" aria-label=${"Character sex: " + note} class=${noteClass}>
    ${note}
  </div>`;
}
...
function imageOnlyTemplate(model) {
  ...
  var reverseFaceHtml = runtime && typeof runtime.renderCardReverseFaceHtml === "function" ? runtime.renderCardReverseFaceHtml() : "";
