...
test("Studio delegates canvas primitives and export to AntV X6", () => {
  ...
  assert.match(editorSource, /router: \{ name: "normal"/)
  assert.match(editorSource, /connector: \{ name: "smooth"/)
  assert.match(editorSource, /new GridLayout/)
  assert.match(editorSource, /grid: false/)
  assert.doesNotMatch(editorSource, /captionPlate|selector: "caption"/)
  assert.match(editorSource, /kind === "activation" \? "#1b7269"/)
  assert.match(editorSource, /kind === "inhibition" \? "#b23a2b" : "#171717"/)
  assert.match(editorSource, /d: "M 0 -10 L 0 10"/)
  assert.match(editorSource, /graph\.getCellsBBox\(genes\)/)
  assert.match(editorSource, /row % 2 === 0 \? offset : columns - 1 - offset/)
