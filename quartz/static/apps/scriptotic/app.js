  ...
  function init(root) {
    ...
    const urlRow = el(
      ...
      el("input", {
        ...
        value: "https://www.youtube.com/watch?v=Bbwp4PbWYzw",
      }),
    )

    const actions = el(
      "div",
      { className: "scr-actions" },
    ...
    function setStatus(txt, cls = "warn") {
      ...
      ui.status.className = `scr-status ${cls}`
    }
    function setProgress(p) {
      ui.bar.style.width = `${Math.max(0, Math.min(100, p))}%`
    }
    ...
    async function startJob() {
      ...
      if (!url) {
        setStatus("Enter a YouTube URL.", "warn")
        return
      }
      ...
      try {
        const r = await api(origin, "/api/transcribe", {
          method: "POST",
        ...
        jobTimer = setInterval(async () => {
          try {
            ...
            setStatus(
              `Status: ${sj.status}`,
              sj.status === "error" ? "err" : sj.status === "done" ? "ok" : "warn",
            )
            ...
            if (sj.status === "done") {
              ...
              if (sj.download_url) {
                const a = el("a", {
                  href: sj.download_url,
                  innerText: "Download transcript",
                  className: "scr-btn",
                })
